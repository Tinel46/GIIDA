...
def generate_dataloader(args):
    ...
    source_dataset = dataset(root=base_path, task=args.source, download=True,
                             transform=MultipleApply([transforms_train_weak, transforms_train_strong]))
    target_dataset = dataset(root=base_path, task=args.target, download=True,
                             transform=MultipleApply([transforms_train_weak, transforms_train_strong]))
    ...
    target_loader_trans_test = torch.utils.data.DataLoader(trans_test_dataset,
                                                           batch_size=args.batch_size * 5, num_workers=args.workers,
